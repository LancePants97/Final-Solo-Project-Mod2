...
RSpec.describe Invoice, type: :model do
  ...
  before(:each) do
    ...
    @ii_111 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_2.id, quantity: 10, unit_price: 15, status: 1)
    @ii_1111 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_3.id, quantity: 15, unit_price: 10, status: 1)

    @ii_2 = InvoiceItem.create!(invoice_id: @invoice_2.id, item_id: @item_2.id, quantity: 10, unit_price: 15, status: 1)
  ...
  describe "instance methods" do
    it "total_revenue" do
      total_rev = (@ii_1.quantity * @ii_1.unit_price) + (@ii_11.quantity * @ii_11.unit_price) + (@ii_111.quantity * @ii_111.unit_price) + (@ii_1111.quantity * @ii_1111.unit_price) # aka (9 * 10) + (13 * 5) + (10 * 15) = 305
  ...
  # item2 qualifies for discount2 (at least 12 items) | discount amount should be 9.75 (NOT 6.5!)
    ...
    it "discounted_revenue" do
      total_rev = (@ii_1.quantity * @ii_1.unit_price) + (@ii_11.quantity * @ii_11.unit_price) + (@ii_111.quantity * @ii_111.unit_price) + (@ii_1111.quantity * @ii_1111.unit_price)
      item1_discount = ((@ii_1.quantity * @ii_1.unit_price) * @discount1.percentage_discount)
      item8_discount = ((@ii_11.quantity * @ii_11.unit_price) * @discount2.percentage_discount)
      total_discount = ((item1_discount + item8_discount) / 100)
      discounted_rev = (total_rev - total_discount)

      expect(@invoice_1.discounted_revenue(@merchant1)).to eq(discounted_rev) # total_revenue(155) - discount_amount(18.75)
    end
    ...
    it "all_merchants_discount_amount" do
      item1_discount = ((@ii_1.quantity * @ii_1.unit_price) * @discount1.percentage_discount)
      item8_discount = ((@ii_11.quantity * @ii_11.unit_price) * @discount2.percentage_discount)
      item3_discount = ((@ii_1111.quantity * @ii_1111.unit_price) * @discount3.percentage_discount)
